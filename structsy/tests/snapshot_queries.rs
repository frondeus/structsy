...
pub fn basic_query() {
    structsy_inst("basic_query", |db| {
        ...
        let count = snapshot.query::<Basic>().fetch().count();
        assert_eq!(count, 1);
        let count = snapshot.query::<Basic>().by_name("aaa".to_string()).fetch().count();
        assert_eq!(count, 1);
        let count = snapshot.query::<Basic>().by_name_str("aaa").fetch().count();
        assert_eq!(count, 1);
        let count = snapshot.fetch(Filter::<Basic>::new()).count();
        assert_eq!(count, 1);
        let count = snapshot
            .fetch(Filter::<Basic>::new().by_name("aaa".to_string()))
            .count();
        assert_eq!(count, 1);
        let count = snapshot.fetch(Filter::<Basic>::new().by_name_str("aaa")).count();
        assert_eq!(count, 1);
        Ok(())
    });
}
...
pub fn basic_range_query_str() {
    structsy_inst("basic_query", |db| {
        ...
        tx.commit()?;
        let snapshot = db.snapshot()?;
        let count = snapshot.query::<Basic>().by_range_str("aaa".."bbb").into_iter().count();
        assert_eq!(count, 1);
        let result = snapshot
            .query::<Basic>()
