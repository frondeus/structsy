...
fn test_ref() {
    structsy_inst("basic_query", |db| {
        ...
        let count = tx.query::<Basic>().by_other(insa.clone()).into_iter().count();
        assert_eq!(count, 1);

        let count = tx.query::<Basic>().by_other_range(insa..=insb).into_iter().count();
        assert_eq!(count, 2);
        //TODO: check this may look weird ... the root query rules ... even if i get he query from
        //the db
        let other_query = db.query::<Other>().by_name("aaa".to_string());
        let count = tx.query::<Basic>().by_other_query(other_query).into_iter().count();
        assert_eq!(count, 1);
        tx.commit()?;
        Ok(())
    });
...
fn test_vec_ref() {
    structsy_inst("basic_query", |db| {
        ...
        let count = tx.query::<BasicVec>().by_other(vec![insa.clone()]).into_iter().count();
        assert_eq!(count, 1);
        ...
        //the db
        let other_query = db.query::<Other>().by_name("aaa".to_string());
        let count = tx.query::<BasicVec>().by_other_query(other_query).into_iter().count();
        assert_eq!(count, 1);
        tx.commit()?;
        Ok(())
    });
